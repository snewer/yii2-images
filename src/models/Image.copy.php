 ...
 * @property $previews
 */
class ImageCopy extends ActiveRecord
{
    ...
    public function upload($source, array $options = [], array $withPreviews = [])
    {
        ...
        if ($image->getCore() instanceof \Imagick) {
            $image->getCore()->setImagePage($image->width(), $image->height(), 0, 0);
        }
        ...
        $supportAC = ($options['supportAC'] === "true" || $options['supportAC'] === true);
        ...
        $resultSource = trim($image->encode($supportAC ? 'png' : 'jpeg', 90));
        $this->_source = $resultSource;

        /* @var $storage \snewer\storage\AbstractStorage */
        $storage = $this->getStorage()->getStorageById($this->originalUploadStorageId);
        $path = $storage->upload($resultSource, $supportAC ? 'png' : 'jpg');
        ...
        $withPreviews[] = [300, 300];
        foreach ($withPreviews as $preview) {
            if (isset($preview[0], $preview[1]) && min($preview[0], $preview[1]) > 0 && !in_array($preview[0] . 'x' . $preview[1], $addedPreviews)) {
